...
// Test harness for the refund handler. The full economy doesn't route refunds yet, so each
// test calls the handler directly inside a database transaction, passing it the same kind of
// transactional handle (`unit`) the real system would.
//
// A refund reverses a sale, so a refund can only be tested against a sale that already
// happened. Each test first runs a real purchase through the `spend` handler; that purchase
// records its money lines in the sale store under an order id, and the refund later looks
// them up by that id and posts their opposites.
//
// The fixture bundles four short helpers — issue starting funds, run a sale, run a refund,
// read a balance — so each test only has to spell out the one thing it is testing.
type Fixture = {
...
function setup(): Fixture {
  ...
  return {
    // Give a user real, spendable funds, the way a top-up would: post the two ledger lines a
    // top-up posts, leaving the named amount in the user's spendable balance for a later sale
    // to draw on.
    ...
    // Move the seller's earned balance into PAYOUT_RESERVE. Both accounts go up when credited and
    // down when debited, so debiting one and crediting the other by the same amount keeps the
    // posting balanced. This leaves earned at zero — the state a seller who has already been paid
    // out is in when a refund of their sale arrives.
...
async function unwindsSellerEarnedAndPlatformFee(): Promise<void> {
  ...
  // After the refund, every account the sale paid into is back to zero: the seller's earned
  // balance, and the platform's revenue account. Revenue returning to zero is the point — the
  // platform's fee on the sale was given back, not kept, because a refund undoes every line
  // the sale posted.
...
// R16: a refund must always make the buyer whole, even when the seller has already paid out the
// cut the sale credited them. The seller is debited only by what is still in their earned balance
// (here nothing), and the uncollectable remainder is booked to RECEIVABLE so the posting balances.
async function refundsBuyerEvenAfterSellerPaidOut(): Promise<void> {
  ...
  // The seller withdraws their whole earned cut before the refund arrives, so the naive
  // sign-flip reversal would overdraw earned and roll the whole refund back.
...
// R17: an order can be reversed at most once. Before posting, a refund takes a one-time lock
// keyed by the order id (`reversed:<orderId>`). The first refund takes the lock and posts the
// reversal; a second refund finds the lock already held and gets handed back the first reversal
// as a duplicate, so the buyer is never credited twice.
async function secondRefundOfSameOrderIsDuplicate(): Promise<void> {
  ...
  // An idempotency key is the value that makes a retried request run at most once. Even with a
  // brand-new key — so the framework's own retry dedup would let this through — the second refund
  // is still blocked, because the first refund already took the per-order claim and only one
  // refund per order may win it.
...
async function refundRevokesBuyerEntitlement(): Promise<void> {
  ...
  // Grant the buyer the SKU the way a real sale does (a sale grants the item it sold), so the
  // refund's revoke has an ownership record to reclaim.
...
// A gift refunds the same as any sale, but ownership is taken back from the RECIPIENT who
// received it (the sale's recorded `recipientId`), not from the buyer who paid. This pins the
// symmetry: the gift granted to the recipient, so the refund revokes from the recipient.
...
// R14/R19: revoke is idempotent — a refund of a sale whose buyer was never granted the SKU still
// commits and simply leaves ownership false, rather than throwing on the absent row.
...
// A blank or whitespace-only orderId is malformed structured input: it names no order to
// reverse. It must throw MALFORMED up front rather than degrade to a silent UNKNOWN_ORDER
// rejected outcome, which would hide the malformed request behind an ordinary "no".
