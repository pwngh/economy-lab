...
it('every seeded page renders with no axe violations', async () => {
  ...
  const mkt = { status, settings, users, subscriptions, flash: null };
  ...
  const searchCheckpoint = checkpoint ? await eco.find(checkpoint.root) : null;
  ...
  const cases: [string, ComponentType<never>, unknown][] = [
    ...
    ['ledger-search-miss', Ledger, { page: ledger, query: 'nope', hit: null }],
    [
      'payouts',
      Payouts,
    ...
    ['market-race', Market, { ...mkt, flash: raceFlash }],
    ['market-paused', Market, { ...mkt, status: paused }],
    ['txn-detail', TxnDetail, { ...txnBase, statement: null, lineage: null }],
    [
      'txn-drill',
      TxnDetail,
      { txnId, account: drillAccount, posting, statement, lineage, checkpoint },
    ],
    ['pipeline', Pipeline, pipelineData],
    ['pipeline-invalid', Pipeline, { ...pipelineData, flash: pipelineInvalid }],
  ...
  for (const [name, Page, data] of cases) {
    expect({ page: name, violations: await violationsOf(Page, data) }).toEqual({
      page: name,
      violations: [],
    });
  }
