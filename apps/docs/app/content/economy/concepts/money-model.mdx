...
`par ≥ payout` (in practice `payout = par`) ties the creator's cash-out to the same value the platform reserved. It's why a credit earned is redeemable for exactly the dollars set aside for it. That's what [solvency](/economy/concepts/solvency/) checks: real USD in trust covers every spendable credit valued at `par`.
