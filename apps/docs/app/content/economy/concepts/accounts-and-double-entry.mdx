...
No posting can create or destroy value: like a balanced scale, its lines cancel within each currency, so the books as a whole always sum to zero. Folding the postings back re-derives the same balance every time.
...
One wrinkle on the table above: with [`PLATFORM_SHARDS`](/economy/reference/configuration/) set past `1`, each hot `SYSTEM` account splits into shard rows, the bare id plus `platform:revenue#1` and so on. A shard row behaves like its parent in every way that matters here (identity checks strip the `#` suffix first, so currency, class, and normal side all come from the base account), and the logical balance is the sum over the rows. [Concurrency](/economy/concepts/concurrency/) explains why the split exists and how a posting picks its shard.
