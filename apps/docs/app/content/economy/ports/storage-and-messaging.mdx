...
sourceRefs: ["src/ports.ts#L272 · Store", "src/ports.ts#L206 · Ledger", "src/adapters/memory.ts#L1167 · memoryStore", "src/engines/postgres.ts#L1579 · postgresStore", "src/worker/relay.ts#L55 · relayOutbox"]
---
...
<Callout type="note">
The in-memory adapter gets atomicity from a per-store undo log rather than a real database transaction: each write records a reverser, and a rollback replays them last-to-first. The observable contract (all-or-nothing) is the same as the SQL engines.
</Callout>
...
The schema is declarative and applied by hand or in CI, never on app startup. The engines fail fast instead: each checks the database's recorded schema version against the code's and refuses to run on a mismatch, rather than silently operating on a drifted shape.
</Callout>
