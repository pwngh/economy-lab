...
The rules that keep money conserved are written once, not re-implemented per backend. They're verified against every adapter, and the SQL engines push them down into the database as well. We come back to that below.
