---
title: 'Rates'
summary: 'The rates port supplying the fixed CREDIT↔USD buy, par, and payout rates.'
order: 30
status: stable
sourceRefs:
  - 'src/ports.ts#L154-L172 · Rates'
  - 'src/ports.ts#L182 · Rate'
  - 'src/adapters/rates.ts#L56-L100 · configuredRates'
related:
...
This port does not track a market. Credits are platform-priced, not market-priced, so the rates are business constants a deployment sets, not quotes from an exchange. There is no FX feed, no refresh, and no clock-driven movement. `configuredRates` returns the same numbers every time it's asked.
