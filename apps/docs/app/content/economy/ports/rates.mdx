...
<Callout type="note">
The adapter doesn't enforce `buy ≥ par ≥ payout` in code. It serves back whatever six numbers you configure. The ordering is a contract on the rate source, not a runtime guard, so a deployment that miswires the constants can break the platform's margin. The money-model page explains [what that ordering protects](/economy/concepts/money-model/#the-invariant-buy--par--payout).
</Callout>
