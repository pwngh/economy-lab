...
sourceRefs:
  [
    'src/ports.ts#L132-L144 · Processor',
    'src/adapters/processor.ts#L67 · httpProcessor',
    'src/adapters/thunes-processor.ts#L127 · thunesProcessor',
    'src/adapters/thunes-processor.ts#L339 · decodeThunesPayoutCallback',
  ]
related:
  [
...
`key` is the [idempotency](/economy/concepts/idempotency/) key that makes a resend safe. The background worker passes the payout saga id here, so a retried submit pays out at most once. `userId` is an opaque `usr_…` token, never personal data. `amount` is in real USD, already converted from the seller's reserved credits at the [payout rate](/economy/concepts/money-model/).

The return is just `{ providerRef }`: the provider's own id for the disbursement, which the lab records on the [payout saga](/economy/concepts/payout-saga/) for the audit trail.
...
<Callout type="warning">
  ...
  one-shot request-and-reference shape, which a real payout rail rarely does.
  For production you write an adapter for your provider's actual API. The
  Thunes adapter below is the worked example.
</Callout>
...
The transaction id minted at step 2 becomes the `providerRef`. Because the id exists before any money moves, there's no "2xx but no reference" ambiguity: the confirm step only has to succeed.
...
- **At-most-once disbursement under retry.** The same `key` must never pay twice, however many times the worker resubmits. Both adapters honor this, the reference adapter by refusing to retry an ambiguous 2xx, Thunes by keying the whole flow on `external_id`.
...
- **No real provider ships with the lab.** `httpProcessor` is the reference adapter and `thunesProcessor` is an integration against one specific rail's documented API. Wiring a provider, holding its credentials, and verifying its sandbox are the host's responsibility.
