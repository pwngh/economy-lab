...
order: 20
status: stable
sourceRefs: ["src/ports.ts#L130-L140 · Processor", "src/adapters/processor.ts#L66 · httpProcessor", "src/adapters/thunes-processor.ts#L120 · thunesProcessor", "src/adapters/thunes-processor.ts#L330 · decodeThunesPayoutCallback"]
related: ["economy/reference/operations/request-payout", "economy/reference/operations/settle-payout", "economy/concepts/solvency"]
---
...
<Callout type="note">
`submitPayout` returns once the provider *accepts* the request, not once the money lands. The disbursement is in flight at that point; the saga stays in `SUBMITTED` until the settlement webhook arrives.
</Callout>
...
`httpProcessor` is a stub. It assumes a provider that already speaks economy-lab's one-shot request-and-reference shape, which a real payout rail rarely does. For production you write an adapter for your provider's actual API. The Thunes adapter below is the worked example.
</Callout>
...
<Callout type="note">
The two Thunes replay error codes (`1007001` for a reused `external_id`, `1007002` for an already-confirmed transaction) are derived from Thunes' documentation. The adapter's comments flag them to verify against the live sandbox before production: an honest limitation of a lab-grade integration.
</Callout>
