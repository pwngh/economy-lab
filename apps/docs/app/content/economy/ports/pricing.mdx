...
sourceRefs:
  [
    ...
    'src/contract.ts#L292 · FeePolicy',
  ]
...
Each seller's share rounds _down_, so the
distributed shares can fall a minor unit or two short of the net. `revenueForSplit` hands it to `REVENUE` along with the fee, so the credits always
