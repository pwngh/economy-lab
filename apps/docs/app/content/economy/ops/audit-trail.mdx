...
order: 2
status: stable
sourceRefs: ['src/ops/audit.ts', 'scripts/audit-verify.ts', 'scripts/audit-worm.ts']
...
example in `scripts/audit-worm.ts` appends through the chained sink to a segment file
