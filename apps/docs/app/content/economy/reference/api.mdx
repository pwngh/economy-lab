...
| `redisRateLimiterFrom` | Adapt an already-connected ioredis client into a fixed-window RateLimiter over INCR and PEXPIRE. |
...
`logger` ports (wrapped by `opsRuntime`), detects twelve incident signatures with plain
...
| `outbox-backlog` | 1 | A targeted relay re-drive (`runOnce({ only: ['relay'] })`), verified against the next gauge. |
...
| Export | What it is |
| --- | --- |
| `opsRuntime` | Wraps a host meter/logger pair, forwarding every call unchanged while buffering a bounded window of signals for the detectors. |
| `OpsRuntime` | What opsRuntime returns: the wrapped meter and logger the composition consumes, plus the signal feed. |
...
| Export | What it is |
| --- | --- |
| `createSupervisor` | Builds the supervisor from its injected deps; tick() runs every pass once, start() ticks on a Scheduler. |
| `defaultSupervisorConfig` | Every threshold, window, cooldown, and cap, with the defaults the passes run under. |
| `SupervisorDeps` | What the host injects: clock, signals, saga source, the levers (runSweep, runRelay, reviveInbox, pauseWorker, prove), the audit sink, and escalate. |
