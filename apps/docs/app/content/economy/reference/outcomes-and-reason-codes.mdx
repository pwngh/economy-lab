...
The three statuses answer three different questions, so it helps to read them as the three ways a
request can land. The operation went through. The operation was a repeat of one already done. The
...
is the whole point of the [throw-versus-decline rule](#rejected-versus-thrown) below, so it's worth
...
they never light up error dashboards or page an on-call engineer. A buyer running out of credits is
