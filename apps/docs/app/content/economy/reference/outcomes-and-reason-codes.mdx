...
The money fields in `detail` — `required`, `available`, `minimum`, `requested`, `spent`, `limit` — are branded [`Amount`](/economy/concepts/money-model/)s, so a caller compares them directly (`detail.available.minor < detail.required.minor`) and formats them with `encodeAmount`. Over the [HTTP service](/economy/reference/http-service/) they travel as the same decimal strings every other wire amount uses.
...
The `detail` names the `account`, the `required` amount that hadn't cleared, and
`availableAt` — when it will have, assuming no further activity — so a caller can tell the
user when to try again.

`RISK_DENIED` is actionable the same way: its `detail` carries what was `spent` in the
window, the `limit`, which window tripped (`class` — `'in'` for top-ups and promo grants,
`'out'` for spends, subscriptions, and payout requests), the `windowMs`, and `retryAfter` —
when the oldest counted attempt ages out of the [velocity window](/economy/concepts/spend-velocity/).
