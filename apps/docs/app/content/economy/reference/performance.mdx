...
sourceRefs:
  [
    'scripts/support/harness.ts#L170 · buildBenchConfig',
    'scripts/support/harness.ts#L129 · curveSizes',
...
| --------- | ------ | ------ | ------------- |
| in-memory | 20,910 | 31,745 | 39,621        |
...
| Backend   | topUp  | spend  | requestPayout |
| --------- | ------ | ------ | ------------- |
| in-memory | 25,063 | 33,299 | 43,468        |
| Postgres  | 260    | 239    | 268           |
| MySQL     | 155    | 123    | 121           |

In memory, `submit` is CPU-bound and runs tens of thousands per second. On a SQL backend each `submit` is its own transaction, with a commit and an `fsync` per operation, so the rate drops to hundreds per second — and concurrency, pipelining independent transactions, is what recovers it.
...
Many small balanced movements — tips, unlocks, creator rewards — are accepted into an in-instance journal and settled to the ledger as one net posting, instead of one `submit` each.
