...
That gap is the reason a signed checkpoint exists. Verifying one in O(accounts) anchors ongoing integrity where a full re-prove — O(postings) — can't keep up once the ledger has years of history. The [background worker](/economy/reference/background-worker/) seals checkpoints on a schedule for exactly this reason.
