...
sourceRefs:
  [
    'src/server.ts#L185 · createServer',
...
import instancePurchaseHttp from '../../../snippets/http/instance-purchase.sh?shiki';
...
| `POST /instances/:scope/purchase` | Hand an in-world purchase to the scope's [fast lane](/economy/concepts/instance-economy/). Routed only when the server is built with `instances`; `404` otherwise. |
...
`POST /instances/:scope/purchase` is the transport an unprivileged game server calls to buy through the [instance economy](/economy/concepts/instance-economy/). It exists only when `ServerOptions.instances` carries the lane manager — an `InstanceEconomies` built over the same ports — and answers `404` otherwise. The route hands the decoded purchase to `laneFor(scope)` and returns the `PurchaseOutcome`; the money settles later, on the lane's own schedule.

<CallTabs name="instance-purchase" http={instancePurchaseHttp}>
...
There is deliberately no batch route: `Economy.submitBatch` and the [submit coalescer](/economy/reference/the-economy/#batches-and-bursts) are in-process surface. An HTTP edge that wants burst-coalescing wraps its own economy handle; the wire protocol stays one operation, one outcome.
