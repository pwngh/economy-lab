...
sourceRefs:
  [
    'src/server.ts#L84 · createServer',
    'scripts/main.ts#L473 · runServe',
    'test/server.test.ts',
  ]
...
The wrapper is <SourceLink to="src/server.ts#L84">createServer</SourceLink>. It takes an `Economy` and returns a handler with the signature `(request: Request) => Promise<Response>`:
