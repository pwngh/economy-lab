...
order: 30
status: stable
sourceRefs: ["src/server.ts#L83 · createServer", "scripts/main.ts#L472 · runServe", "test/server.test.ts"]
...
<Callout type="note">
This page documents the HTTP edge: the routes, the wire format, and the webhook gate. It does not re-document what each operation *means*; that lives with the operations. The service decodes a body and hands it to `submit`; everything past that boundary is the in-process economy.
</Callout>
...
<Callout type="note">
The raw body is read exactly once. Verification, the replay decode, and the handler all work over that one buffer. The server rebuilds a fresh `Request` from the bytes so the handler can re-read the body the server already consumed for signing.
</Callout>
...
<Callout type="warning">
The deployed process fails fast on missing secrets. `requireSecrets` rejects an unset or blank `SIGNING_SECRET` or `WEBHOOK_SECRET` at startup, naming each one, so an unset `WEBHOOK_SECRET` can't quietly leave inbound callbacks unverified. The full set of environment variables lives on [configuration](/economy/reference/configuration/).
</Callout>
