...
summary: 'Grant ownership of a SKU to an account.'
order: 100
status: stable
sourceRefs:
  [
    'src/operations/entitlements.ts#L41 · grantEntitlement',
    'src/operations/registry.ts#L45 · REGISTRY',
    "src/contract.ts#L137 · kind 'grantEntitlement'",
    'src/contract.ts#L41 · EntitlementAttrs',
...
The grant always succeeds. It overwrites any previous record for the same user and SKU, and has no
prerequisite: the user need not already own anything, and no balance is checked.
...
before the handler runs. Granting names an arbitrary `userId` the caller need not own, and it posts no debit. The
gate therefore stands in for the ownership check the posting path would otherwise apply.

## Reason codes

`grant-entitlement` returns no reason codes: it has no `rejected` path. It fails only by throwing.
