...
order: 100
status: stable
sourceRefs: ["src/operations/entitlements.ts#L41 · grantEntitlement", "src/operations/registry.ts#L45 · REGISTRY", "src/contract.ts#L137 · kind 'grantEntitlement'", "src/contract.ts#L41 · EntitlementAttrs", "src/economy.ts#L795 · RESTRICTED_TO_PRIVILEGED"]
...
<Callout type="note">
  The core stores `attrs` with the grant; it does not act on them. `expiresAt`, for example, is kept
  as data. No [sweep](/economy/reference/background-worker/) expires an entitlement the way the worker
  expires a promo grant. Reading and enforcing these fields is the host's job.
</Callout>
...
- No balance moves, so the ledger stays balanced and conserved. A grant can never unbalance the
  books.
