...
A `top-up` can return one [reason code](/economy/reference/outcomes-and-reason-codes/) as a
`rejected` outcome:
...
The pause gate applies only to `user` actors, and a `user` can't call `top-up`, so a top-up is
