...
order: 140
status: stable
sourceRefs: ["src/operations/reverse.ts#L39-L76 · reverse", "src/contract.ts#L170-L178 · kind 'reverse'"]
...
You cannot reverse a reversal. A reversal records `kind: "reverse"` in its metadata, and the handler refuses any `txnId` that carries that marker. Undoing an undo would loop the same money out and back in, and let an operator chain reversals to move a balance at will.
</Callout>
