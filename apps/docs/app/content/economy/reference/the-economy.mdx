...
sourceRefs:
  [
    'src/index.ts#L78 · createEconomy',
    'src/economy.ts#L92 · createEconomy',
    'src/contract.ts#L415 · Economy',
  ]
related:
  [
...
A host that wants the whole stack from one call uses `boot`, which opens the ports, assembles the economy over them, and builds the [background worker](/economy/reference/background-worker/) over the same bag (pass `worker: false` to skip it):
