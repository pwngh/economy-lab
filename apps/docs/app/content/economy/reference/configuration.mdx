...
related:
  [
    ...
    'economy/ports/storage-and-messaging',
...
lifecycle](/economy/concepts/lifecycles/) for why the holding period exists.
...
A hot platform account (`REVENUE`, `STORED_VALUE`, `TRUST_CASH`, ...) is one row, and concurrent postings of the same kind queue on its lock. `PLATFORM_SHARDS` splits each one across several rows so those postings spread out; [concurrency](/economy/concepts/concurrency/) covers which accounts shard and how a posting picks its row.
...
to [storage and messaging](/economy/ports/storage-and-messaging/), read by
