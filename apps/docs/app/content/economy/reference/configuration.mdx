...
order: 40
status: stable
sourceRefs: ["src/config.ts#L111-L174 · loadConfig", "src/index.ts#L127-L231 · capabilitiesFromEnv", "scripts/main.ts#L141-L207 · productionExternals", ".env.example"]
related: ["economy/ports/rates", "economy/ports/processor", "economy/ports/storage-and-messaging", "economy/reference/http-service"]
---
...
<Callout type="note">
`.env.example` ships variable names only, never values. Copy it to `.env` and fill it in.
`.gitignore` excludes every `.env*` file so a real secret never lands in the repo.
</Callout>
...
The serve and worker entry point (`scripts/main.ts`) refuses to start if either secret is
blank, in *any* `NODE_ENV`, a stricter check than `loadConfig`'s production-only one.
</Callout>
...
<Callout type="note">
Either bound left unset or non-integer leaves that bound `null`, which reads as "no window
configured", distinct from a real epoch value of zero. So a half-set pause never half-activates.
</Callout>
