...
order: 35
status: stable
sourceRefs: ["src/worker/index.ts#L57-L68 · SWEEP_NAMES", "src/worker/payouts.ts#L49 · settleDuePayouts", "src/worker/relay.ts#L55 · relayOutbox"]
related: ["economy/reference/http-service", "economy/reference/operations/request-payout", "economy/reference/configuration", "economy/reference/performance"]
---
...
<Callout type="note">
`drainInbox` is never gated on the economy pause. A provider settlement the rail already confirmed must keep flowing through a maintenance window, so the inbox keeps draining even when discretionary user writes are refused.
</Callout>
