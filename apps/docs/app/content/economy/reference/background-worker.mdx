...
sourceRefs:
  [
    ...
    'src/worker/payouts.ts#L50 · settleDuePayouts',
...
| Sweep              | What it does                                                                                     |
| ------------------ | ------------------------------------------------------------------------------------------------ |
| `payouts`          | Advance each due payout [saga](/economy/concepts/lifecycles/) one step.                          |
...
| `relay`            | Drain the [outbox](/economy/ports/storage-and-messaging/) to the dispatcher.                     |
...
```ts
const summary = await settleDuePayouts(store, ctx, { now, limit });
...
The relay drains the [outbox](/economy/ports/storage-and-messaging/), where each event was written in the same transaction as the money move it describes. It claims up to `limit` events, sends each through the dispatcher, and marks the delivered ones done.
