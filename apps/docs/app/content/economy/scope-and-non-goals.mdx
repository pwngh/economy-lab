...
status: stable
sourceRefs:
  ["README.md#L29 · a lab, not a product", "src/ports.ts#L130-L140 · Processor", "src/ports.ts#L118-L121 · Dispatcher", "src/ports.ts#L154-L172 · Rates"]
...
<Callout type="note">
Everything here is a non-goal *by design*, not a gap waiting to be filled. The subject of study is the ledger and its invariants, so the rails underneath stay thin on purpose.
</Callout>
...
The stubbed processor approves payouts unconditionally. That is fine for studying the [payout saga](/economy/concepts/lifecycles/), and exactly wrong for anything holding real funds.
</Callout>
...
<Callout type="note">
The [risk gates](/economy/concepts/actors-and-authorization/) the lab *does* implement (spend velocity, settlement maturity) are application-layer fraud controls, not the regulated compliance plumbing above. They guard the ledger; they are not KYC or sanctions screening.
</Callout>
