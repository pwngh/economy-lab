...
.DEFAULT_GOAL := help
.PHONY: help up down env bootstrap db-migrate db-clean test coverage check prove fuzz prop smoke bench bench-queue bench-prod scale trace demo demo-ops demo-ops-integrity demo-ops-deadlock backup restore-drill audit-verify ledger-verify ci dev start worker samples

help: ; @printf '%s\n' 'targets:' '  up down env bootstrap db-migrate db-clean' '  test coverage check prove fuzz prop smoke bench bench-queue bench-prod scale trace demo samples' '  demo-ops demo-ops-integrity demo-ops-deadlock' '  dev start worker' '' 'make bootstrap  = env + up + wait for health + db-migrate (one-step setup)' 'make env        = append compose-local DB URLs from .env.compose into .env (idempotent; never clobbers your values)' 'make db-migrate = apply the schema for $$DATABASE_URL (or .env) via psql/mysql' 'make db-clean   = drop orphaned throwaway namespaces a killed run left behind (el_* schemas/databases)' 'make coverage   = the test suite with per-file line/branch coverage' 'make prop       = property-based ledger laws with a shrinking counterexample search (in-memory)' 'make bench      = throughput + integrity bench (in-memory + any reachable DB), reseeded per run' 'make bench-prod = run the bench inside a Linux container vs the compose DBs (production-parity fsync)' 'make bench-queue = one unattended sequential bench session (cool-down + rig canary) into a stamped bench-results/ dir' 'make scale      = per-subject scale probe across backends (does per-op cost stay flat as history grows?)' 'make samples    = compile the docs fenced ts samples against the real entry points' 'make demo-ops   = ops supervisor demos: stuck-saga closed loop; -integrity = tamper escalation; -deadlock = retry storm (needs DATABASE_URL; make bootstrap first)' 'make backup     = data-only dump of every env-named engine into backups/ (pg_dump/mysqldump)' 'make restore-drill = restore the newest dump into a scratch el_drill_* namespace and prove it' 'make audit-verify FILE=<jsonl> = re-derive a hash-chained ops audit trail and report the first break' 'make ledger-verify FILE=<jsonl> KEY=<hex public key> = offline chain + checkpoint verification of a read.export file' 'make ci         = the whole github ci locally: the check and apps jobs; DB=1 adds the db matrix'

up:         ; sh scripts/docker.sh up -d
down:       ; sh scripts/docker.sh down
env:        ; sh scripts/env-merge.sh
