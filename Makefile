...
.DEFAULT_GOAL := help
.PHONY: help up down bootstrap db-migrate test check prove fuzz trace demo dev start worker

help: ; @printf '%s\n' 'targets:' '  up down bootstrap db-migrate' '  test check prove fuzz trace demo' '  dev start worker' '' 'make bootstrap  = up + wait for health + db-migrate (one-step setup)' 'make db-migrate = apply the schema for $$DATABASE_URL (or .env) via psql/mysql'
