...
.DEFAULT_GOAL := help
.PHONY: help up down bootstrap db-migrate db-clean test coverage check prove fuzz prop smoke bench bench-prod scale trace demo demo-ops demo-ops-integrity demo-ops-deadlock backup restore-drill audit-verify ledger-verify ci dev start worker samples
